...
WHERE 1=1
  ...
  select * from NSADMIN0.DBO.T_LOG(NOLOCK) WHERE LOG_ID = 'HYANGA' and log_03 = '125.136.170.219' and log_04 > '2021-08-01'
  ...
  using NSADMIN0.DBO.T_LOG(NOLOCK) b
	...
	MERGE INTO T_RUN A
	 ...
	 WHEN MATCHED THEN 
	  ...
	  ( select distinct first_Value(log_04) over(partition by  CONVERT(varchar(10), LOG_04, 120) order by log_04) log_04, log_id , log_01 from   NSADMIN0.DBO.T_LOG(NOLOCK) where log_01 = 'hyanga002') B
	  WHERE A.RUN_98 = B.LOG_01
	  AND A.RUN_ID = B.LOG_ID
	    AND A.RUN_99 >'20210801'
	  AND B.LOG_04 >'2021-08-01'
	  and run_98 = 'hyanga002'
	  AND B.LOG_ID = 'HYANGA'
	 ...
	 DECLARE @random3 INT = ROUND(((60)*RAND()),0)
	MERGE INTO T_RUN A
	 USING  ( select distinct first_Value(log_04) over(partition by  CONVERT(varchar(10), LOG_04, 120) order by log_04) log_04, log_id , log_01 from   NSADMIN0.DBO.T_LOG(NOLOCK) where log_01 = 'hyanga002') B
	   ON A.RUN_98 = B.LOG_01
	  AND A.RUN_ID = B.LOG_ID
	  AND A.RUN_99 >'20210801'
       and run_98 = 'hyanga002'
	 ...
	 select distinct first_Value(log_04) over(partition by  CONVERT(varchar(10), LOG_04, 120) order by log_04) log_04, log_id from   NSADMIN0.DBO.T_LOG(NOLOCK) where log_01 = 'hyanga002'
	...
	MERGE INTO T_ODM A
	 USING  ( select distinct first_Value(log_04) over(partition by  CONVERT(varchar(10), LOG_04, 120) order by log_04) log_04, log_id , log_01 from   NSADMIN0.DBO.T_LOG(NOLOCK) where log_01 = 'hyanga002' and LOG_04 >'2021-11-01') B
	   ON A.ODM_98 = B.LOG_01
	  AND A.ODM_ID = B.LOG_ID
	  AND A.ODM_99 >'2021-11-01'
       and ODM_98 = 'hyanga002'
   ...
   DECLARE @random3 INT = ROUND(((5)*RAND()),0)
	MERGE INTO T_LOT A
	 USING  ( select distinct last_Value(log_04) over(partition by  CONVERT(varchar(10), LOG_04, 120) order by CONVERT(varchar(10), LOG_04, 120)) log_04, log_id , log_01 from   NSADMIN0.DBO.T_LOG(NOLOCK) where log_01 = 'hyanga002' and log_04 > '2021-08-01') B
	...
	select * from  NSADMIN0.DBO.T_LOG where log_id ='hyanga'
   ...
   DECLARE @ran INT = ROUND(((60)*RAND()),0)
	select dateadd(mi,@ran,'17:20')
	 ...
	 select distinct last_Value(log_04) over(partition by  CONVERT(varchar(10), LOG_04, 120) order by CONVERT(varchar(10), LOG_04, 120)) from  NSADMIN0.DBO.T_LOG where log_id ='hyanga' and log_01 = 'hyanga002' and log_04 >'2021-11-01'
	...
	MERGE INTO T_LOT A
	 ...
	 WHEN MATCHED THEN 
	  ...
	  select * from NSADMIN0.DBO.T_LOG where log_04 > '2021-11-28' and log_id ='HYANGA'
